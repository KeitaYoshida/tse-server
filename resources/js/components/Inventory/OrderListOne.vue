  ...
  <div id="makepdf" class="a4-area" v-if="item_list">
    <v-card class="a4" v-for="(items, index) in item_list" :key="index">
      <v-card-title primary-title>
        ...
        <span>TA 8106N1050E000 {{ cnt_order_code }}</span>
        <v-spacer></v-spacer>
        <span class="pagenum">{{ index + 1 }} / {{ item_list.length }} ページ</span>
      </v-card-title>
      <v-data-table
        :headers="headers"
        :items="items"
      ...
      >
        <template v-slot:items="props">
          <td>{{ props.item.assy_code !== null ? props.item.assy_code.slice(0,10) : "" }}</td>
...
export default {
  ...
  data: function() {
    return {
      headers: [
        { text: "親品目", value: "assy_code", align: "center" },
        ...
        { text: "合計金額", value: "price", align: "center" }
      ],
      item_list: null
    };
  },
  created: function() {
    axios.get("/inventory/order-list-one/" + this.cnt_order_code).then(res => {
